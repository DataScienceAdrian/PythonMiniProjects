...
def colourPop(canvas):
    ...
    if not canvas.data.cropPopToHappen:
...
def getPixel(event, canvas):
    ...
    try:
        if canvas.data.colourPopToHappen == True and \
                canvas.data.cropPopToHappen == False and canvas.data.image is not None:
            data = []
...
def rotateFinished(canvas, rotateWindow, rotateSlider, previousAngle):
    if canvas.data.rotateWindowClose:
        rotateWindow.destroy()
        canvas.data.rotateWindowClose = False
    else:
        if canvas.data.image is not None and rotateWindow.winfo_exists():
            canvas.data.angleSelected = rotateSlider.get()
            if canvas.data.angleSelected is not None and canvas.data.angleSelected != previousAngle:
...
def performSolarize(canvas, solarizeWindow, solarizeSlider, previousThreshold):
    if canvas.data.solarizeWindowClose:
        ...
        if solarizeWindow.winfo_exists():
            ...
            if canvas.data.image is not None and threshold_ != previousThreshold:
...
def desktopBk(canvas):
    if canvas.data.image is not None:
        ...
        newLocation = os.path.dirname(canvas.data.imageLocation) + "/desktopPhoto.bmp"
...
def buttonsInit(root, canvas):
    ...
    colourPopButton = Button(toolKitFrame, text="Colour Pick",
                             background=backgroundColour,
                             width=buttonWidth, height=buttonHeight,
                             command=lambda: colourPop(canvas))
    colourPopButton.grid(row=4, column=0)
    mirrorButton = Button(toolKitFrame, text="Mirror Image",
                          background=backgroundColour,
                          width=buttonWidth, height=buttonHeight,
                          command=lambda: mirror(canvas))
    mirrorButton.grid(row=5, column=0)
    flipButton = Button(toolKitFrame, text="Flip 180 stop.",
                        background=backgroundColour,
                        width=buttonWidth, height=buttonHeight,
                        command=lambda: flip(canvas))
    flipButton.grid(row=6, column=0)
    transposeButton = Button(toolKitFrame, text="Flip 90 stop.",
